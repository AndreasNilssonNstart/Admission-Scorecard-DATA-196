...
-- CRB.SSN=A.SSN and datediff(day,A.ReceivedDate,CRB.Date) between -30 and isnull(datediff(day,A.ApplicationDate,A.DisbursedDate),60)
  ...
  LEFT JOIN   [Reporting-db].[nystart].[CreditReportsBase] CBR ON CBR.SSN = A.SSN
    ...
    -- it can sometimes result in matching ore than one report to an application, in that case we pick the newest one 
    --and max(CBR.Date) group by CRB.SNN
    
    and A.ApplicationID =7985637
