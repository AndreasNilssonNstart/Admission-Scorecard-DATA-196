-- Step 1: Get Max CBR.Date and Min UCScore for each ApplicationID
WITH MaxDates AS (
    SELECT
        A.ApplicationID,
    ...
    WHERE
        A.Status = 'DISBURSED' 
        AND A.HasCoapp = 1 
        AND A.CoappSameAddress = 0 -- Modified here to get different address
...
FROM 
    [Reporting-db].[nystart].[Applications] as A
INNER JOIN MaxDates MD ON A.ApplicationID = MD.ApplicationID AND A.UCScore = MD.MinUCScore
LEFT JOIN [Reporting-db].[nystart].[CreditReportsBase] CBR ON CBR.SSN = A.SSN AND CBR.Date = MD.MaxDate

WHERE
    A.Status = 'DISBURSED'
    AND A.CoappSameAddress = 0
ORDER BY 
    A.ApplicationID ASC;