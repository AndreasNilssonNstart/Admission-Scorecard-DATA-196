...
snn_score_co AS (
    ...
    row_number() over (partition by AccountNumber,A.SSN order by CBR.Date desc) as RowNumber ,
    ...
    WHERE
        A.HasCoapp = 1  and A.[IsMainApplicant] = 0
...
FROM snn_score_co s inner join DEL90 as d  on d.AccountNumber = s.AccountNumber


WHERE  s.RowNumber = 1    --and d.Ever90 = 1  AND  s.AccountNumber = 7739188  AND
